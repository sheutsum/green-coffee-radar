...
Catalog: /category/green/78/?page=N
Product:  /product/<slug>/<id>/category/78/display/1/
...
class CoffeeMeUpScraper(Cafe24Scraper):
    ...
    # 2026-09-08: 생두 카테고리가 182 -> 78 로 옮겨가면서 182 가 404. 슬러그(green)는
    # Cafe24 rewrite 가 무시하고 번호만 본다 — 번호가 바뀌면 여기만 갈아끼우면 된다.
    catalog_url_template = "{base}/category/green/78/?page={page}"
    # 슬러그가 "생두" 또는 "파나마"/"코스타리카"처럼 산지명만 시작하기도 함.
    # 안전하게 '/product/' 포함 + 카테고리 78 URL로 충분.
